...
export default function JobCard({ job, index, onClick, isCompact = false }) {
  const stageColors = {
  ...
  return (
    <Draggable draggableId={job.id} index={index}>
      {(provided, snapshot) => (
        ...
        >
          <Card 
            className={`p-2 text-xs shadow-sm hover:shadow-md transition-shadow cursor-grab active:cursor-grabbing border-l-4 ${statusColor} ${snapshot.isDragging ? 'opacity-50 shadow-xl scale-105' : ''}`}
            onClick={(e) => {
                // Prevent click when dragging
                if(!snapshot.isDragging) onClick(job);
            }}
          >
